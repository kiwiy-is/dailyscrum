...
const Page = async ({ params, searchParams }: Props) => {
  ...
  if (!user) {
    const returnPath = `/daily-scrum/orgs/${org.hash_id}/join?code=${codeParamValue}`;
          ...
          <CardFooter>
            <div className="flex gap-2 w-full">
              <Link
                ...
                href={`/daily-scrum/sign-up?return-path=${encodeURIComponent(
                  returnPath
                )}`}
              >
                Sign up
              ...
              <Link
                ...
                href={`/daily-scrum/sign-in?return-path=${encodeURIComponent(
                  returnPath
                )}`}
              >
                Sign in
  ...
  if (member) {
    return (
      <div>
        ...
        <br />
        <Link href={`/daily-scrum/orgs/${orgHashId}`}>Go to Workspace</Link>
      </div>
    );
  }
